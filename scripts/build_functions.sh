...
build_aom() {
	...
	[ "$ARCH" = "x86" ] && ASM_F=(-DENABLE_NASM=OFF)
...
build_xvidcore() {
	...
	if [ "$ARCH" = "riscv64" ]; then
		echo "[*] Replacing config.sub for riscv64..."
		curl -fsSL -o config.sub 'https://git.savannah.gnu.org/gitweb/?p=config.git;a=blob_plain;f=config.sub;hb=HEAD' || exit 1
	fi
...
build_xavs2() {
	...
	if [ "$ARCH" = "riscv64" ]; then
		echo "[*] Patching config.sub for riscv64..."
		curl -fsSL -o config.sub 'https://git.savannah.gnu.org/gitweb/?p=config.git;a=blob_plain;f=config.sub;hb=HEAD' || exit 1
	fi
...
build_vvenc() {
	...
	if [[ "$ARCH" == "armv7" ]] || [[ "$ARCH" = "riscv64" ]]; then
		...
		# warning suppression for RISC-V
		if [[ "$ARCH" = "riscv64" ]]; then
			warning_flags+=(
